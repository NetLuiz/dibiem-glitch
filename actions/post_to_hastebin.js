module.exports = {
	...
	short_description:  "Posts your code to Hastebin",
	...
	html: function (isEvent, data) {
		...
		<span class="embed-desc">Version: ${this.version}</span>
		</div>
		</div><br>
		<div style="width: 95%; padding-top: 8px;">
		Code to Post:<br>
		<textarea id="code" rows="4" placeholder="Write a code here or use variables..." style="width: 95%; font-family: monospace; white-space: nowrap; resize: none;"></textarea>
		</div><br>
	...
	action: function (cache) {
		...
		var _this = this; // To fix some error
		const data = cache.actions[cache.index];
		const code = _this.evalMessage(data.code, cache);
		const storage = parseInt(data.storage);
		const varName = _this.evalMessage(data.varName, cache);

		if (!code) return console.log("[Post to Hastebin] You need to write something to post.");

		// Main code
		var WrexMODS = _this.getWrexMods(); // Using Wrex mods, as always.
		const hastebin = WrexMODS.require('hastebin.js');
		const haste = new hastebin();
		
		haste.post(`${code}`, function (resultt) {
					if(resultt !== undefined) {
					_this.storeValue(resultt, storage, varName, cache);
				}
